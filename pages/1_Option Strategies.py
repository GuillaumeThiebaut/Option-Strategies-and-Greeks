...
if st.session_state.show_inputs:
    ...
    if option_strategy == 'Strangle':
        ...
        if premium_activated:
            ...
            premium_1 = 0
            premium_2 = 0 
        call = option_payoff(S, K1, premium_1, 'Call', f'{position_investor}')
        put = option_payoff(S, K2, premium_2, 'Put', f'{position_investor}')
        payoff = call + put
    ...
    if option_strategy == 'Put Ladder':
        ...
        if premium_activated:
            ...
            premium_1 = premium_2 = premium_3 = 0 
        put_3 = option_payoff(S, K1, premium_1, 'Put', f'{opposition_position}')
        put_3 = option_payoff(S, K2, premium_2, 'Put', f'{opposition_position}')
        put_3 = option_payoff(S, K3, premium_3, 'Put', f'{position_investor}')
        payoff = put_3 + put_3 + put_3
        
        # Creating the plot
        fig, ax = plt.subplots(figsize=(8, 5))
        ax.plot(S, payoff, label="Payoff", color="black", linewidth=1.5)
        ax.plot(S, call_1, '--', label=f"{opposition_position} Put", color="blue", alpha=0.6)
        ax.plot(S, call_2, '--', label=f"{opposition_position} Put", color="red", alpha=0.6)
        ax.plot(S, call_3, '--', label=f"{position_investor} Put", color="green", alpha=0.6)
    ...
    if option_strategy == 'Strap':
        ...
        fig, ax = plt.subplots(figsize=(8, 5))
        ax.plot(S, payoff, label="Payoff", color="black", linewidth=1.5)
        ax.plot(S, call, '--', label=f"{position_investor} Two Calls", color="blue", alpha=0.6)
        ax.plot(S, put, '--', label=f"{position_investor} Put", color="red", alpha=0.6)
    ...
    if option_strategy == 'Strip':
        ...
        ax.plot(S, payoff, label="Payoff", color="black", linewidth=1.5)
        ax.plot(S, call, '--', label=f"{position_investor} Call", color="blue", alpha=0.6)
        ax.plot(S, put, '--', label=f"{position_investor} Two Puts", color="red", alpha=0.6)
